...
import NewLiveColumnPanel, {
	referrers,
} from '../shared/panels/NewLiveColumnPanel'
...
import {
	getLiveColumnsForTable,
	getLiveColumnLinksOnTable,
} from '../../utils/config'
import {
	filterIcon,
	blistIcon,
...
const compileLiveColumnDataForTable = (table, config) => {
	...
	const linkColumns = getLiveColumnLinksOnTable(
		table.schema,
		table.name,
		config
	)
...
const getColumnWidths = (table, liveColumns) => {
	...
	for (const col of table.columns) {
		...
		const numColTypeChars = (
			liveColumnData?.givenName || displayColType(col.data_type)
		).length
...
function TablesBody(props, ref) {
	...
	const [liveDataPanelReferrer, setLiveDataPanelReferrer] = useState(
		referrers.ADD_LIVE_DATA
	)
	...
	const sortRules = useMemo(
		() => defaultSortRules(primaryKeyColNames),
		[primaryKeyColNames]
	)

	// Live column info.
	const liveColumns = useMemo(
		() => compileLiveColumnDataForTable(table, config),
		[table, config]
	)
	const hasLiveColumns = useMemo(
		() =>
			Object.keys(((config?.tables || {})[schema] || {})[table.name] || {})
				.length > 0,
		[config, schema, table]
	)
	...
	const columnWidths = useMemo(
		() => getColumnWidths(table, liveColumns),
		[table, liveColumns]
	)
	...
	const fetchNextPage = useCallback(async () => {
		...
		if (
			nextPageFetchOffset.current &&
			records.length <= nextPageFetchOffset.current
		) {
			return
		}
	...
	useEffect(() => {
		if (props.seedCursor) {
			...
			if (
				...
				seedCursor.current.id !== props.seedCursor.id
			) {
				...
				if (isFirstSeed.current) {
					...
					fadeInTimer.current = setTimeout(() => {
						...
						removeAccentTimer.current = setTimeout(() => {
							$(`.${pcn('__row')}--new`).removeClass(
								`${pcn('__row')}--new-accent`
							)
						}, 500)
					}, 10)
				}
